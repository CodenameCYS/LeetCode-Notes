...
class Solution:
    def checkIfPrerequisite(self, n: int, prerequisites: List[List[int]], queries: List[List[int]]) -> List[bool]:
        requirements = {i:set() for i in range(n)}
        for a,b in prerequisites:
            # print(a,b)
            requirements[b].add(a)
            requirements[b].update(requirements[a])
            for c in requirements[a]:
                requirements[b].update(requirements[c])
            for k,v in requirements.items():
                if b in v:
                    v.update(requirements[b])
        ans = []
        for a, b in queries:
            ans.append(a in requirements[b])
        return ans