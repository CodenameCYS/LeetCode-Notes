...
Constraints:
    1. 1 <= userId, itemId <= 5 * 104
    2. 1 <= bidAmount, newAmount <= 109
    3. At most 5 * 104 total calls to addBid, updateBid, removeBid, and getHighestBidder.
    4. The input is generated such that for updateBid and removeBid, the bid from the given userId for the given itemId will be valid.
'''
# === 324ms && 50.99MB === #
class AuctionSystem:
    ...
    def addBid(self, userId: int, itemId: int, bidAmount: int) -> None:
        if userId in self.memory[itemId]:
            self.updateBid(userId, itemId, bidAmount)
        else:
            self.memory[itemId][userId] = bidAmount
            bisect.insort(self.pool[itemId], (bidAmount, userId))
        return

    def updateBid(self, userId: int, itemId: int, newAmount: int) -> None:
        oldAmount = self.memory[itemId][userId]
        self.pool[itemId].pop(bisect.bisect_left(self.pool[itemId], (oldAmount, userId)))
        self.memory[itemId][userId] = newAmount
        bisect.insort(self.pool[itemId], (newAmount, userId))
        return

    def removeBid(self, userId: int, itemId: int) -> None:
        oldAmount = self.memory[itemId][userId]
        self.pool[itemId].pop(bisect.bisect_left(self.pool[itemId], (oldAmount, userId)))
        self.memory[itemId].pop(userId)
        return

    def getHighestBidder(self, itemId: int) -> int:
        return self.pool[itemId][-1][1] if len(self.pool[itemId]) > 0 else -1
