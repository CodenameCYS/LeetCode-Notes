...
Constraints:
    ...
    5. 1 <= int(events[i][1]) <= 105
    6. The number of id<number> mentions in any "MESSAGE" event is between 1 and 100.
    7. 0 <= <number> <= numberOfUsers - 1
    8. It is guaranteed that the user id referenced in the OFFLINE event is online at the time the event occurs.
'''
# === 38ms && 18MB === #
class Solution:
    def countMentions(self, numberOfUsers: int, events: List[List[str]]) -> List[int]:
        ...
        for event in events:
            ...
            if event[0] == "MESSAGE":
                ...
                uid = int(event[2])
                status[uid] = t + 60
            # print(f"msg = {event[0]}, status = {status}, cnt = {cnt}")
        return cnt
