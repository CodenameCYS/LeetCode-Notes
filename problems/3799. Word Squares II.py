        ...
        def dp(idx, status, selected):
            ...
            for i in range(n):
                if status & (1<<i) != 0:
                    continue
                if idx == 0:
                    dp(idx+1, status | (1<<i), tuple(selected + [words[i]]))
                elif idx == 1 and selected[0][0] == words[i][0]:
                    dp(idx+1, status | (1<<i), tuple(selected + [words[i]]))
                elif idx == 2 and selected[0][3] == words[i][0]:
                    dp(idx+1, status | (1<<i), tuple(selected + [words[i]]))
                elif idx == 3 and selected[1][3] == words[i][0] and selected[2][3] == words[i][3]:
                    dp(idx+1, status | (1<<i), tuple(selected + [words[i]]))
            return 
