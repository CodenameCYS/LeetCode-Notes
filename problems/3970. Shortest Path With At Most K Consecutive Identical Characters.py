...
Constraints:
    1. 1 <= n == labels.length <= 5 * 104
    2. 0 <= edges.length <= 5 * 104
    3. edges[i] == [ui, vi, wi]
    4. 0 <= ui, vi <= n - 1
    5. ui != vi
    6. 1 <= wi <= 104
    7. labels consists of lowercase English letters
    8. 1 <= k <= 50
'''
# === 170ms && 48.55MB === #
class Solution:
        ...
        while q:
            ...
            for v, w in graph[u]:
                if labels[u] == labels[v]:
                    ...
                    if (v, cnt+1) in seen:
                        continue
                    heapq.heappush(q, (tot + w, v, cnt+1))
                else:
                    if (v, 1) in seen:
                        continue
                    heapq.heappush(q, (tot + w, v, 1))
        return -1