...
bool is_sub_path(struct ListNode* head, struct TreeNode* root){
    if(head == NULL){
        return true;
    }
        ...
        return is_sub_path(head->next, root->left) || is_sub_path(head->next, root->right);
    }
}
// === 24ms && 13.6MB === //
bool isSubPath(struct ListNode* head, struct TreeNode* root){
    ...
    if(root -> val == head -> val && is_sub_path(head, root)){
        return true;
    }
    else{
