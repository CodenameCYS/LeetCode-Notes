...
Constraints:
    ...
    6. patterni contains at least one lowercase English letter and at most 5 '*' characters.
...
class Solution:
    def minCost(self, source: str, target: str, rules: list[list[str]], costs: list[int]) -> int:
        n = len(source)
        costs = [c + Counter(p)["*"] for (p, r), c in zip(rules, costs)]

        @lru_cache(None)
        def can_replace(idx, pattern, replace):
            m = len(pattern)
            if idx + m > n:
                return False
            elif target[idx:idx+m] != replace:
                return False
            for ch1, ch2 in zip(source[idx:idx+m], pattern):
                if ch2 != "*" and ch1 != ch2:
                    return False
            return True
        ...
        def dp(idx):
            if idx >= n:
                return 0
            ans = math.inf if source[idx] != target[idx] else dp(idx+1)
            for cost, (pattern, replace) in zip(costs, rules):
                if can_replace(idx, pattern, replace):
                    ans = min(ans, cost + dp(idx + len(pattern)))
            return ans
