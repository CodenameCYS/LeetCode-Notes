...
Constraints:
    1. 1 <= n == nums1.length == nums0.length <= 105
    2. 0 <= nums1[i], nums0[i] <= 104
    3. nums1[i] + nums0[i] > 0
    4. The total sum of all elements in nums1 and nums0 does not exceed 2 * 105.
'''
MOD = 10**9+7
# === 215ms && 36.76MB === #
class Solution:
    def maxValue(self, nums1: list[int], nums0: list[int]) -> int:
        ones = [(x, y) for x, y in zip(nums1, nums0) if y == 0]
        nums = [(x, y) for x, y in zip(nums1, nums0) if y != 0]
        nums = ones + sorted(nums, key=lambda x: (-x[0], x[1]))

        ans = 0
        for one, zero in nums:
            num = (pow(2, one, mod=MOD) - 1) * pow(2, zero, mod=MOD)
            num = num % MOD
            ans = ((ans * pow(2, one+zero, mod=MOD)) % MOD + num) % MOD
        return ans