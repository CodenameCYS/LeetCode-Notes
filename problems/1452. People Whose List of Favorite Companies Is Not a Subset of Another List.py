...
class Solution:
    def peopleIndexes(self, favoriteCompanies: List[List[str]]) -> List[int]:
        favoriteCompanies = sorted([(i, set(fc)) for i, fc in enumerate(favoriteCompanies)], key=lambda x: len(x[1]), reverse=True)
        # print(favoriteCompanies)
        ans = []
        for i, it in enumerate(favoriteCompanies):
            is_sub = False
            fc = it[1]
            for idx in ans:
                if fc.issubset(favoriteCompanies[idx][1]):
                    is_sub = True
                    break
            if not is_sub:
                ans.append(i)
        return sorted([favoriteCompanies[idx][0] for idx in ans])