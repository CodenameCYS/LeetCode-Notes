    ...
    def displayTable(self, orders: List[List[str]]) -> List[List[str]]:
        ...
        for name, table, food in orders:
            if table not in record.keys():
                record[table] = {}
            if food not in record[table].keys():
                record[table][food] = 1
            else:
                record[table][food] += 1
            foodset.add(food)
        record = sorted(record.items(), key = lambda x: int(x[0]))
        foodset = sorted(foodset)
        ans = []
        ans.append(["Table"] + foodset)
        for table, order in record:
            ans.append([table] + [str(order.get(food, 0)) for food in foodset])
        return ans