...
Constraints:
    1. 1 <= monsters.length <= 5 * 104
    2. 1 <= monsters[i] <= 109
    3. 0 <= boosts.length <= 5 * 104
    4. boosts[i] == [li, ri, vi]
...
class Solution:
    def minInitialStrength(self, monsters: list[int], boosts: list[list[int]]) -> int:
        ...
        for l, r, v in boosts:
            bonus[l] += v
            bonus[r+1] -= v
        bonus = list(accumulate(bonus))
        strength = 0
        for i in range(n-1, -1, -1):
            monster, boost = monsters[i], bonus[i]
            strength = max(strength + monster if strength > 0 else 0, monster - boost)
        return strength
