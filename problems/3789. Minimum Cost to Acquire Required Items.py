...
Constraints:
    1. 1 <= cost1, cost2, costBoth <= 106
    2. 0 <= need1, need2 <= 109
'''
# === 0ms && 17.26MB === #
class Solution:
    def minimumCost(self, cost1: int, cost2: int, costBoth: int, need1: int, need2: int) -> int:
        ans = min(cost1 * need1 + cost2 * need2, costBoth * max(need1, need2))
        if need1 <= need2:
            ans = min(ans, costBoth * need1 + cost2 * (need2-need1))
        else:
            ans = min(ans, costBoth * need2 + cost1 * (need1-need2))
        return ans