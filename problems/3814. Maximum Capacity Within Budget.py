...
Constraints:
    1. 1 <= n == costs.length == capacity.length <= 105
    2. 1 <= costs[i], capacity[i] <= 105
    3. 1 <= budget <= 2 * 105
'''
# === 437ms && 41.31MB === #
class Solution:
    def maxCapacity(self, costs: List[int], capacity: List[int], budget: int) -> int:
        pairs = [(cost, cap) for cost, cap in zip(costs, capacity) if cost < budget]
        pairs = sorted(pairs)
        i, j = 0, len(pairs)-1
        ans = max([x[1] for x in pairs]) if len(pairs) > 0 else 0
        candi = [0]
        while i <= j:
            while i < j and pairs[i][0] + pairs[j][0] < budget:
                bisect.insort(candi, pairs[i][1])
                i += 1
            ans = max(ans, candi[-1] + pairs[j][1])
            j -= 1
        ans = max(ans, candi[-1] + candi[-2]) if len(candi) > 1 else ans
        return ans
