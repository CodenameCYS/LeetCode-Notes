...
Constraints:
    1. 1 <= value.length == decay.length <= 105
    2. 1 <= value[i], decay[i] <= 109​​​​​​​
    3. 1 <= m <= 109
'''
MOD = 10**9+7
# === 861ms && 32.40MB === #
class Solution:
    def maxTotalValue(self, value: list[int], decay: list[int], m: int) -> int:
        ...
        def get_total_value(num):
            ans = 0
            for val, dec in zip(value, decay):
                if val < num:
                    continue
                t = (val-num) // dec + 1
                ans = (ans + (val + val-dec*(t-1)) * t // 2) % MOD
            return ans

        def get_elems_less_than_num(num):
            def fn(val, dec):
                if val < num:
                    return val
                t = (val-num) // dec + 1
                return max(val-dec*t, 0)
            return [fn(val, dec) for val, dec in zip(value, decay)]
            
        if count_selection(0) <= m:
            return get_total_value(0)
        elif m == 1:
            return max(value)
        ...
        while r-l>1:
            x = (l+r) // 2
            if count_selection(x) <= m:
                r = x
            else:
                l = x
        cnt = count_selection(r)
        if cnt == m:
            return get_total_value(r)
        else:
            tot = get_total_value(r)
            extra = sorted(get_elems_less_than_num(r), reverse=True)
            return (tot + sum(extra[:m-cnt])) % MOD
