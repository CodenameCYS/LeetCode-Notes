...
class Solution:
    def arrangeWords(self, text: str) -> str:
        tokens = text.split()
        tokens = [(i, w) for i, w in enumerate(tokens)]
        tokens = sorted(tokens, key=lambda x: (len(x[1]), x))
        text = " ".join([x[1] for x in tokens]).lower()
        return chr(ord(text[0]) + ord('A')-ord('a')) + text[1:]