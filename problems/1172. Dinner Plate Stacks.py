...
class DinnerPlates:
    ...
    def push(self, val: int) -> None:
        if self.empty_list:
            self.stack[self.empty_list[0]].append(val)
            self.empty_list.pop(0)
        else:
            if self.stack == [] or len(self.stack[-1]) == self.capacity:
        ...
        try:
            ...
            if index == self.stack_size - 1:
                while self.stack != [] and self.stack[-1] == []:
                    self.stack.pop()
                self.stack_size = len(self.stack)
            bisect.insort(self.empty_list, index)
            # print("empty list :: ", self.empty_list)
            self.empty_list = self.empty_list[:bisect.bisect_left(self.empty_list, self.stack_size)]
        except:
            ans = -1
        return ans
