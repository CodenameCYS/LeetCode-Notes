...
Constraints:
    1. 2 <= period <= 109
    2. 1 <= lights.length <= 104
    3. 1 <= lights[i] <= period - 1
    4. 1 <= arrivalTime.length <= 105
    5. 1 <= arrivalTime[i] <= 109
'''
# === 85ms && 38.08MB === #
class Solution:
    def minPenalty(self, period: int, lights: list[int], arrivalTime: list[int]) -> int:
        ...
        def query(t):
            t = t % period
            return 0 if t < ml else period-t
        
        return max(query(t) for t in arrivalTime)